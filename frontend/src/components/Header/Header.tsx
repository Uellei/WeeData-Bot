...
export function Header({ searchItem, botName }: HeaderProps) {
  ...
  return (
    <header className='fixed top-0 w-full z-50 bg-white'>
      <nav className='bg-white pt-7 border-gray-200 px-4 lg:px-6 dark:bg-[#131313] z-50'>
        <div className='flex flex-wrap pb-5 border-b items-center mx-auto max-w-screen-2xl justify-between'>
          <a href="/" className='flex items-center'>
            <img src="" alt="" />
            <span className='self-center text-3xl mr-10 font-semibold whitespace-nowrap dark:text-white lg:text-none'>SEILA LINDA</span>
          </a>
          <div className='flex-grow items-center flex'>
            ...
            <div className={`${menuOpen ? 'block' : 'hidden'} justify-between ml-10 items-center w-full lg:flex lg:w-auto lg:order-1`}>
              <ul className='flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0'>
                ...
                <li>
                  <a href="/kabum" className='text-2xl block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700'>Kabum</a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className={`${searchVisible ? 'flex' : 'hidden'} justify-end w-96 lg:flex lg:w-auto lg:order-1 relative mt-4 lg:mt-0`}>
            <input type='text' className='w-96 h-8 rounded-full pl-4 text-lg items-center justify-center relative' placeholder='Fone de ouvido' onChange={handleInputChange}/>
